 ...
 * In Solr cloud mode Log Feeder will manage and listen a ZooKeeper connection. If there are too many Log Feeder nodes,
 * that can mean it requires a lot of client connections. (for static URLs, use "solr_urls" field, for Solr cloud mode use "zk_connect_string")
...
public class OutputSolr extends Output<LogFeederProps, InputMarker> {
  ...
  private void initParams(LogFeederProps logFeederProps) throws Exception {
    ...
    logger.info(String.format("Config: Number of workers=%d, splitMode=%s, splitInterval=%d. %s",
        workers, splitMode, splitInterval, getShortDescription()));

    implicitRouting = logFeederProps.isSolrImplicitRouting();
    if (implicitRouting) {
  ...
  private void createOutgoingBuffer() {
    ...
    outgoingBuffer = new LinkedBlockingQueue<>(bufferSize);
  }
  ...
  private void pingSolr(int count, SolrClient solrClient) {
    try {
      ...
      if (response.getStatus() == 0) {
        ...
        logger.warn(String.format("Ping to Solr server failed. It will check again. worker=%d, collection=%s, response=%s",
            count, collection, response));
      }
    } catch (Throwable t) {
      logger.warn(String.format("Ping to Solr server failed. It will check again. worker=%d, collection=%s", count, collection), t);
    }
  }
  ...
  class SolrWorkerThread extends Thread {
      ...
      while (true) {
        ...
        try {
          ...
          if (!localBuffer.isEmpty() &&
            ...
            if (isDrain() && !response) {
              logger.warn("In drain mode and sending to Solr failed. Exiting. output=" + getShortDescription());
              break;
            }
          }
          if (localBuffer.isEmpty()) {
          ...
          LogFeederUtil.logErrorMessageByInterval(logMessageKey, "Caught exception in main loop. " + outputData, t, logger, Level.ERROR);
        }
      }
    ...
    private boolean sendToSolr(OutputData outputData) {
      ...
      while (!isDrain()) {
        try {
          ...
          try {
            logger.warn("Solr is not reachable. Retrying after " + RETRY_INTERVAL + " seconds. output=" + getShortDescription(), exception);
          ...
          LogFeederUtil.logErrorMessageByInterval(logMessageKey, "Error sending log message to server. Dropping logs", serverException, logger, Level.ERROR);
          resetLocalBuffer();
          break;
        }
      } 
    ...
    private void createSolrDocument(OutputData outputData) {
      ...
      for (String name : outputData.jsonObj.keySet()) {
        ...
        try {
          ...
          LogFeederUtil.logErrorMessageByInterval(logMessageKey, "Error calculating byte size. object=" + obj, t, logger, Level.ERROR);
        }
      }
    ...
    private void addRouterField() {
      try {
        // W Solr 9.8.0 nie ma już ZkStateReader – używamy getClusterStateProvider() z CloudSolrClient.
        DocCollection docCollection = ((CloudSolrClient) solrClient)
                                        .getClusterStateProvider()
                                        .getClusterState()
                                        .getCollection(collection);
        Collection<Slice> slices = docCollection.getSlices();
        List<String> shards = slices.stream()
                                      .map(Slice::getName)
        ...
        String logMessageKey = this.getClass().getSimpleName() + "_SOLR_CLUSTER_STATE_ERROR";
        LogFeederUtil.logErrorMessageByInterval(logMessageKey, "Error getting cluster state from Solr. output=" + getShortDescription(), e, logger, Level.ERROR);
      }
    }
